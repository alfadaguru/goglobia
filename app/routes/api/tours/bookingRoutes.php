...
$router->post('/api/tours/booking/submit', function () use ($db) {
    ...
    try {
        ...
        if ($promoCodeStr !== '' && function_exists('promoResolveForBooking')) {
            $pr = promoResolveForBooking($db, $promoCodeStr, (float) $finalTotalWithTax, 'tours', (string) $baseCurrency, [
        ...
        if (!empty($promoCodeStr) && $promoDiscount > 0 && $promoData && function_exists('recordPromoUsage')) {
            recordPromoUsage($db, $promoData, (string) $invoiceId, $userId ?? null, $email ?? null, (float) $promoDiscount, 'tours', (string) $baseCurrency);
        }
