    ...
    if (isset($_FILES['room_images']) && !empty($_FILES['room_images']['name'][0])) {
        ...
        for ($i = 0; $i < $file_count; $i++) {
            if ($files['error'][$i] === UPLOAD_ERR_OK) {
                ...
                // user filename) — the extension was taken from $_FILES['name'], so
                // an admin could upload shell.php into the web-served
                // /uploads/hotels/rooms/ dir (RCE where PHP exec isn't blocked;
                ...
                if (!$chk['ok']) { continue; }
                $new_filename = 'room_' . time() . '_' . bin2hex(random_bytes(6)) . '_' . $i . '.' . $chk['ext'];
                $upload_path = $upload_dir . $new_filename;
