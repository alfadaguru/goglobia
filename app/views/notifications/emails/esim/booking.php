...
<div style="background: #ecfdf5; border-left: 4px solid #10b981; padding: 20px 32px; margin: 0;">
    ...
    <div style="font-size: 13px; color: #047857;">Your payment for the eSIM has been successfully processed.</div>
</div>
<?php else: ?>
<div style="background: #fffbeb; border-left: 4px solid #f59e0b; padding: 20px 32px; margin: 0;">
    ...
    <div style="font-size: 13px; color: #b45309;">Your eSIM reservation request has been received and is currently pending payment.</div>
</div>
<?php endif; ?>
...
<div style="padding: 32px; background: #f8fafc;">
    ...
    <div style="font-size: 14px; color: #4a5568; margin-bottom: 32px; line-height: 1.6;">
        <?= ($paymentStatus === 'paid')
            ? "We've received your payment for eSIM booking <strong>#$invoiceId</strong>. Your eSIM is now being prepared — activation details will appear on your invoice."
            : "Thank you for choosing us! We've received your eSIM booking request <strong>#$invoiceId</strong>. Please complete your payment to activate your SIM."
        ?>
    </div>
    ...
    <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; margin-bottom: 24px;">
        <div style="background: #2d3748; color: #ffffff; padding: 12px 16px;">
            <div style="font-size: 15px; font-weight: 600;"><?= htmlspecialchars($package_name ?? 'eSIM Package') ?></div>
            <div style="font-size: 12px; opacity: 0.9;">📍 <?= htmlspecialchars($country ?? '') ?></div>
        </div>
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                <td style="padding: 12px 16px; border-right: 1px solid #e2e8f0; width: 50%;">
                    <div style="font-size: 11px; color: #718096; text-transform: uppercase; font-weight: 600;">Booking ID</div>
                    <div style="font-size: 14px; font-weight: 600; color: #2d3748;">#<?= htmlspecialchars($invoiceId) ?></div>
                </td>
                <td style="padding: 12px 16px; width: 50%;">
                    <div style="font-size: 11px; color: #718096; text-transform: uppercase; font-weight: 600;">Module</div>
                    <div style="font-size: 14px; font-weight: 600; color: #2d3748;">eSIM</div>
                </td>
            </tr>
        </table>
    </div>
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                ...
                <td style="padding: 8px 0; color: #2d3748; font-weight: 600; text-align: right; font-size: 14px;">
                    <?= htmlspecialchars($currency ?? 'USD') ?> <?= number_format((float)($finalTotal ?? 0), 2) ?>
                </td>
            </tr>
            <tr>
    ...
    <div style="text-align: center; margin: 32px 0;">
        <a href="<?= $invoiceUrl ?>" style="display: inline-block; background: <?= ($paymentStatus === 'paid') ? '#10b981' : '#f59e0b' ?>; color: #ffffff; padding: 14px 32px; border-radius: 6px; text-decoration: none; font-size: 14px; font-weight: 600;">
            View Invoice & Activation Details
        </a>
    </div>
    ...
    <div style="background: <?= ($paymentStatus === 'paid') ? '#ecfdf5' : '#eff6ff' ?>; border: 1px solid <?= ($paymentStatus === 'paid') ? '#d1fae5' : '#bfdbfe' ?>; border-radius: 8px; padding: 20px; margin-top: 24px;">
        <div style="font-size: 13px; color: <?= ($paymentStatus === 'paid') ? '#065f46' : '#1e3a8a' ?>; line-height: 1.6;">
            ...
            <?= ($paymentStatus === 'paid')
                ? "Your eSIM activation QR code and setup instructions are available on your invoice page. Open the invoice on your phone to scan the QR code directly."
                : "To activate your eSIM booking, please complete the payment using the button above. Your SIM will be issued automatically once payment is confirmed."
            ?>
        </div>
    </div>
</div>
