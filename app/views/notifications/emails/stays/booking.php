...
<div style="background: #ecfdf5; border-left: 4px solid #10b981; padding: 20px 32px; margin: 0;">
    ...
    <div style="font-size: 13px; color: #047857;">Your payment has been successfully processed and your booking is confirmed.</div>
</div>
<?php else: ?>
<div style="background: #fffbeb; border-left: 4px solid #f59e0b; padding: 20px 32px; margin: 0;">
    ...
    <div style="font-size: 13px; color: #b45309;">Your booking request has been received. Please complete payment to confirm your reservation.</div>
</div>
<?php endif; ?>
...
<div style="padding: 32px; background: #f8fafc;">
    ...
    <div style="font-size: 14px; color: #4a5568; margin-bottom: 32px; line-height: 1.6;">
        <?= ($paymentStatus === 'paid') 
            ? "We've received your payment for hotel booking <strong>#$invoiceId</strong>. Your stay is now officially confirmed!" 
            : "Thank you for choosing us! We've received your hotel booking request <strong>#$invoiceId</strong>. Please complete your payment to secure your reservation." 
        ?>
    </div>
    ...
    <div style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; margin-bottom: 24px;">
        <div style="background: #2d3748; color: #ffffff; padding: 12px 16px;">
            <div style="font-size: 15px; font-weight: 600;"><?= htmlspecialchars($hotelName) ?></div>
            <div style="font-size: 12px; opacity: 0.8;"><?= htmlspecialchars($hotelAddress) ?></div>
        </div>
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                <td style="padding: 12px 16px; border-right: 1px solid #e2e8f0; width: 50%;">
                    <div style="font-size: 11px; color: #718096; text-transform: uppercase; font-weight: 600;">Check-in</div>
                    <div style="font-size: 14px; font-weight: 600; color: #2d3748;"><?= date('D, M d, Y', strtotime($checkin)) ?></div>
                </td>
                <td style="padding: 12px 16px; width: 50%;">
                    <div style="font-size: 11px; color: #718096; text-transform: uppercase; font-weight: 600;">Check-out</div>
                    <div style="font-size: 14px; font-weight: 600; color: #2d3748;"><?= date('D, M d, Y', strtotime($checkout)) ?></div>
                </td>
            </tr>
        </table>
        <div style="padding: 12px 16px; border-top: 1px solid #e2e8f0; font-size: 13px; color: #4a5568;">
            <strong><?= $nights ?> Nights</strong> • <?= htmlspecialchars($roomType) ?> • For <?= $adults ?> Adults<?= $children > 0 ? ", $children Children" : "" ?>
        </div>
    </div>
    ...
    <div style="background: <?= ($paymentStatus === 'paid') ? '#ecfdf5' : '#eff6ff' ?>; border: 1px solid <?= ($paymentStatus === 'paid') ? '#d1fae5' : '#bfdbfe' ?>; border-radius: 8px; padding: 20px; margin-top: 24px;">
        <div style="font-size: 13px; color: <?= ($paymentStatus === 'paid') ? '#065f46' : '#1e3a8a' ?>; line-height: 1.6;">
            ...
            <?= ($paymentStatus === 'paid') 
                ? "Please keep this confirmation handy. You can download your vouchers from the link above. We wish you a pleasant stay!" 
