...
class i18n {
     ...
     * @var string
     */
    protected $cachePath = './cache/i18n/';
    ...
    public function init() {
        ...
        foreach ($this->userLangs as $priority => $langcode) {
            ...
            $searchedPaths[] = $this->langFilePath;
        ...
        if ($this->appliedLang == NULL) {
            $errorMsg = 'No language file was found. Searched paths: ' . implode(', ', $searchedPaths);
        ...
        if ($outdated) {
            ...
            // Best-effort only: on macOS/XAMPP Apache (daemon) can write the
            // cache but chmod() often fails with "Operation not permitted".
    ...
    protected function compile($config, $prefix = '') {
        ...
        foreach ($config as $key => $value) {
            if (is_array($value)) {
                ...
                // PHP reserves `class` for ClassName::class — skip so cache stays loadable.
                if (strcasecmp($fullName, 'class') === 0) {
                    continue;
                }
