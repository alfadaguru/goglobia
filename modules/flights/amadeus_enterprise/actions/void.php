...
// ============================================================================
// AMADEUS ENTERPRISE — VOID / CANCEL FLIGHT ORDER
// ============================================================================
//
// ENDPOINT: POST /flights/amadeus_enterprise/void
...
$router->post('flights/amadeus_enterprise/void', function () use ($db) {
    if (ob_get_level()) {
        ob_end_clean();
    }
    ob_start();
    ...
    try {
        ...
        // Never issued / not confirmed — local cancel only.
        if (empty($booking['pnr']) || in_array((string)$booking['booking_status'], ['failed', 'pending'], true)) {
            $db->update('bookings', [
                'booking_status' => 'cancelled',
                'cancellation_request' => 1,
                'cancellation_status' => 1,
                'cancellation_response' => 'Voided locally on ' . date('Y-m-d H:i:s') . ' (no confirmed airline order)',
            ], ['invoice_id' => $invoiceId]);

            echo json_encode([
                'status' => true,
                'message' => 'Booking cancelled locally. No confirmed airline order to void.',
                'invoice_id' => $invoiceId,
                'pnr' => $booking['pnr'] ?? '',
            ], JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
            exit;
        ...
        if (empty($orderId)) {
            $db->update('bookings', [
                ...
                'cancellation_request' => 1,
                'cancellation_status' => 1,
                'cancellation_response' => 'Voided locally on ' . date('Y-m-d H:i:s') . ' (no Amadeus order id)',
            ], ['invoice_id' => $invoiceId]);
        ...
        $supportLogText = "STEP: void\n"
            . "Environment: {$selectedEnv}\n"
            . "REQUEST:\nDELETE {$cancelUrl}\n\n"
            . "RESPONSE:\nHTTP {$httpCode}\n"
            . (is_string($response) ? $response : json_encode($responseData));

        // Success, or order already gone (1797) — treat as cancelled.
        if (in_array($httpCode, [200, 204], true) || $errorCode === '1797') {
            ...
            echo json_encode([
                ...
                'message' => $errorCode === '1797'
                    ...
                    : 'Flight booking voided successfully',
                'invoice_id' => $invoiceId,
                'order_id' => $orderId,
                'pnr' => $booking['pnr'],
                'http_code' => $httpCode,
